  ...
  return (
    <div className="border-b border-border last:border-0">
      <button
        ...
        onClick={() => setOpen((o) => !o)}
        data-testid="button-faq-item"
      >
        <span className="font-medium text-sm leading-snug">{question}</span>
        {open ? <ChevronDown className="w-4 h-4 shrink-0 rotate-180 transition-transform" /> : <ChevronDown className="w-4 h-4 shrink-0 transition-transform" />}
      </button>
      {open && (
